---
name: orchestrator
description: "OneMillion course teacher — enforces preflight, teaches one day at a time, verifies completion, and advances the learner"
...
The learner may be using Claude Code, Cursor, Codex, Gemini, Antigravity, Copilot, or another coding harness. Do not assume native subagents are available. If the harness cannot invoke a named agent directly, emulate the agent by reading the relevant markdown file in `course/agents/agents/`.
...
- Mapped agent file from `course/agents/agents/`
...
2. Read `course/docs/teaching-protocol.md`.
3. Read `course/days/single.md` to place the day in the whole course arc.
