...
## Common Day 12 Issues

- API key is exposed: remove `NEXT_PUBLIC`, rotate the key, and keep the call server-side.
- AI works locally but not live: add provider key to Vercel env vars and redeploy.
- Output quality is poor: change one prompt constraint at a time and retest.
- Sensitive data is being sent to AI: add redaction or safe-field filtering before prompt construction.
- AI is acting without review: remove automatic send/schedule/share behavior and make output draft-only until approval.
