...
```text
course/docs/qa/persona-simulation-report.md
```
...
- made `one-million-builders/my-onemillion-build` the explicit workspace model
...
```bash
python3 -m json.tool course/course-manifest.json >/tmp/course-manifest.checked
python3 course/tools/course_quality_check.py
python3 course/tools/simulate_course.py
python3 course/tools/simulate_personas.py --report course/docs/qa/persona-simulation-report.md
```
...
The canonical rule lives in `course/docs/teaching-protocol.md`, and `simulate_personas.py` now checks for the required teaching-surface terms.
